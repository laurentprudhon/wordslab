    ...
    public static class AdminHelpers
    {
        ...
        public static int RunAsSudo(string commandLine)
        {
            ...
            using (Process process = Process.Start(startInfo))
            {
                ...
                {
                    throw new Exception("Error while waiting for process exit");
                }
        ...
        public static unsafe bool IsProcessElevated()
        {
            ...
            using (token)
            {
                ...
                if (!Interop.Advapi32.GetTokenInformation(
                    token,
                    (uint)Interop.Advapi32.TOKEN_INFORMATION_CLASS.TokenElevation,
                    new SafeLocalAllocHandle(new IntPtr(&elevation)),
